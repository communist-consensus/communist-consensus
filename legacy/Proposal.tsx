...
const Proposal = ({ id }: { id: string }) => {
  ...
  return (
    <React.Fragment>
      <NewSolutionDialog
        show={show_new_solution}
        onClose={() => set_show_new_solution(false)}
      />
        ...
        <AccordionDetails>
          ...
          {proposal.computed_final_solution_id ? (
            <Stack>
              ...
              <Solution
                id={proposal.computed_final_solution_id}
                proposal_id={proposal.id}
                conference_id={proposal.computed_final_conference_id}
              />
            </Stack>
          ) : null}
          <Divider light />
          <ConferenceSelect
            n_round={proposal.computed_n_round}
            proposal_id={proposal.id}
          />
          {proposal.status === ProposalStatus.discussing_voting ? (
            <Button onClick={() => set_show_new_solution(true)}>
              新增解决方案
            </Button>
          ) : null}
        </AccordionDetails>
...
const ConferenceSelect = (props: { n_round: number; proposal_id: string }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const solutions = await get_solutions(
        props.proposal_id,
        round,
        selected_conference,
        page,
        ...
        <Stack>
          {solutions.map((solution) => (
            <Solution
              ...
              conference_id={selected_conference}
            />
          ))}
        </Stack>
      </Stack>
...
const NewSolutionDialog = (props: { show: boolean; onClose: Function }) => {
