import { ActionBundle, Actions, ActionSignatures, ActionSubjects, BlockContext, ConferenceID, DBBlock, DBConference, DBConferenceSolutionPair, DBDomainProposalPair, DBPeer, DBProposal, DBSolution, DomainEntity, DomainID, ExtendedSolution, InitialParams, IPFSAddress, PeerJSON, Profile, ProposalID, RIConfig, SolutionID } from '../shared/types';
import { decode, encode, encode_to_str, RSA_sign, uint8array_to_b64, uint8array_to_utf8 } from '../shared/utils';
...
export async function request<T, R>(url: string, data?: T) {
  const mid = window.ri_config ? window.ri_config.my_peer_json.id : undefined;
  ...
  if (mid) {
    ...
    signature = encode_to_str(await RSA_sign(window.peer.privKey, encode(token)));
  }
  return await(
    await fetch(url, {
      mode: 'cors',
...
export async function post_init(
...
export async function get_sub_domains(domain_id: DomainID, page: number): Promise<{
...
export async function get_domains(page: number): Promise<{
  domains: DomainEntity[],
  total: number,
  n: number,
}> {
...
export async function get_blocks(page: number): Promise<{
  blocks: DBBlock[],
  total: number,
  n: number;
}> {
...
export async function get_latest_block(): Promise<DBBlock> {
  return (
    await request<{}, { block: DBBlock }>(`${url_prefix}/latest_block`)
  ).block;
}
...
export async function ipfs_add(x: any) {
  return (await request<{}, { cid: string }>(`${url_prefix}/ipfs/add`, {
    content: encode_to_str(x),
  })).cid;
}

export async function get_conferences(proposal_id: string, round_id: number, page = 1) {
  return (
...
export async function vote_solution(proposal_id: string, conference_id: string, solution_id: string) {
  return await request<{}, {}>(
    `${url_prefix}/proposal/${proposal_id}/conference/${conference_id}/solution/${solution_id}/vote`,
  );
}

export async function has_vote_solution(proposal_id: string, conference_id: string, solution_id: string) {
  return (
...
export async function get_solution_votes(proposal_id: string, conference_id: string, solution_id: string) {
  return (
...
export async function get_solutions(proposal_id: string, round_id: number, conference_id: string, page = 1) {
  return (
...
export function get_config() {
  const config: RIConfig = JSON.parse(localStorage.getItem('riconfig'));
  return config;
}

export async function init_global() {
  const config = get_config();
  window.ri_config = config;
  if (config) {
    window.peer = await PeerId.createFromJSON(config.my_peer_json);
  }
}

export function set_config(config: RIConfig) {
  localStorage.setItem('riconfig', JSON.stringify(config));
}
...
export async function comment_solution(proposal_id: ProposalID, conference_id: ConferenceID, solution_id: SolutionID, content_cid: string) {
  return await request<{}, {}>(
