...
const blockchains = new Map<NodeID, BlockChain>();
...
app.post('/resume', async (req, res) => {
  ...
  // TODO verify
  const config: ConsensusConfig = JSON.parse(body.config);
  const blockchain = await init_p2p(config, { clear_db: false });
  blockchains.set(config.peer_id_json.id, blockchain);
...
app.post(
  '/proposal/:proposal_id/round/:round_id/conferences/:page',
  async (req: Req, res) => {
    res.json({
      code: 0,
...
app.post('/peer/:mid', async (req: Req, res) => {
  res.json({
    peer: encode_to_str(await req.blockchain.db.peer.get_peer(req.params.mid)),
  });
});
...
app.post(
  '/proposal/:proposal_id/conference/:conference_id/solution/:solution_id/comment',
  async (req: Req, res) => {
