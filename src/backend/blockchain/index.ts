...
export default class BlockChain extends EventEmitter {
  ...
  private async broadcast_actions_testimony(
    ...
    const signature = await RSA_sign(
      this.peer_id.privKey,
      encode(at),
    );
  ...
  private async init(ctx: Context) {
    ...
    this.on(ConsensusEvent.witness_broadcast_end, (block_ctx: BlockContext) => {
      this.ctx.log(`witness broadcast end`, block_ctx.action_bundle);
    });
    this.on(ConsensusEvent.actions_broadcast_end, (block_ctx: BlockContext) => {
  ...
  public async start(
    ...
    if (opt) {
      ...
      const actions = this.ctx.pending_block
        ? this.ctx.pending_block.next.actions
        : [];
      ...
      this.ctx.log('mode: resume', pending_block.cycle_id, latest_block.epoch);
      ...
      prev.next.next = {
        ...
        epoch: pending_block.cycle_id + 1,
  ...
  async on_internal_actions_broadcast_start(block_ctx: BlockContext) {
    ...
    this.emit(ConsensusEvent.actions_broadcast_start, block_ctx);
    ...
    this.actions_cache = [];
    this.emit(ConsensusEvent.before_actions_broadcast, block_ctx);
    if (block_ctx.actions.length) {
      await this.broadcast_actions_testimony(block_ctx, {
        ...
        node_id: this.peer_id.toB58String(),
      });
    }
    await this.sleep_second(block_ctx.actions_broadcast_window_end - get_now());
    block_ctx.state = BlockCtxState.actions_broadcast_end;
    this.emit(ConsensusEvent.actions_broadcast_end, block_ctx);
  }
  ...
  async on_internal_witness_broadcast_start(block_ctx: BlockContext) {
    ...
    this.ctx.ee.once(
      ConsensusEvent.internal_final_witness_testimony_cid,
      final_witness_testimony_cid_cb,
    );

    this.emit(ConsensusEvent.witness_broadcast_start, block_ctx);
    ...
    while (true) {
      ...
      if (n_tries > 0) {
        this.emit(ConsensusEvent.witness_broadcast_retry, block_ctx, n_tries);
        ...
        if (idx && !action_subjects.find((i) => i === this.mid_b58)) {
      ...
      this.emit(
        ConsensusEvent.witness_broadcast_before_end,
        block_ctx,
        n_tries,
      );
    ...
    this.emit(ConsensusEvent.witness_broadcast_end, block_ctx);
    ...
    await this.db.add_block(block_ctx);

    // TODO 事务 & apply_actions 到 写入数据库期间不执行其他任务
    this.emit(ConsensusEvent.after_apply_actions, block_ctx);
    ...
    block_ctx.next.next = {
      ...
      prev: block_ctx.next,
      n_peer, // 仅用于广播参数的计算
      epoch: block_ctx.next.epoch + 1,
      state: BlockCtxState.actions_broadcast_start,
      actions_broadcast_window_start: block_ctx.witness_broadcast_window_end,
    };

    this.emit(ConsensusEvent.witness_broadcast_start_finished, block_ctx);
