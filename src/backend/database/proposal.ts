...
export default (ctx: Context, manager: EntityManager): IDBProposal => {
  // TODO
  function gen_id() {
   ...
   * vote_solution
   */
  async function commit_solution(
    mid: NodeID,
    ...
    await Promise.all(
      solution.tasks.map(async (task, idx) =>
    ...
    const proposal = await manager.findOne(ProposalEntity, proposal_id);
    ...
    const conference = await manager.findOne(ConferenceEntity, conference_id);
    ...
    await vote_solution(mid, proposal_id, conference_id, solution_id);
  }
    ...
    await manager.update(
      ...
      },
      {
        ...
        computed_publicity_end:
          round_start +
          computed_discussion_voting_duration +
          n_participant_to_publicity_duration(n_participant, ctx.N),
      },
    );
   ...
   */
  async function set_proposal_properties(
    mid: NodeID,
    proposal_id: ProposalID,
    ...
    await update_proposal_computed({
  ...
  async function vote_solution(
    ...
    const max_n_vote = (await manager.findOne(ConferenceEntity, conference_id))
      .computed_max_n_vote;
  ...
  async function get_which_conference(
    proposal_id: ProposalID,
    round_id: number,
    mid: NodeID,
  ) {
  ...
  return {
    ...
    async get_conference_solutions(
      proposal_id: ProposalID,
      round_id: number,
      conference_id: ConferenceID,
      page: number,
    ...
    async activate(proposal_id: ProposalID) {
      await set_proposal_status(proposal_id, ProposalStatus.discussing_voting);
    },

    async get_proposal(proposal_id: ProposalID) {
      return await manager.findOne(ProposalEntity, proposal_id);
    },

    async get_proposal_status(proposal_id: ProposalID) {
      const proposal = await manager.findOne(ProposalEntity, proposal_id);
      return proposal ? proposal.status : undefined;
    },

    async add_proposal_comment(
      mid_b58: NodeID,
      proposal_id: ProposalID,
    ...
    async get_n_participant(proposal_id: ProposalID) {
      return (
    ...
    async has_solution(solution_id: SolutionID) {
      return !!(await manager.count(SolutionEntity, {
        id: solution_id,
      }));
    },

    async has_proposal(proposal_id: ProposalID) {
    ...
    async add_proposal(
      ...
      await conference_add_peers([mid], proposal_id, conference_id, round_id);
      await set_proposal_properties(mid, proposal_id, proposal.properties);
      await commit_solution(mid, proposal_id, proposal.default_solution, true);
    },

    commit_solution,

    async freeze(proposal_id: ProposalID) {
      await set_proposal_status(proposal_id, ProposalStatus.freezed);
    },
    ...
    async withdraw_voting(
      ...
      if (
        !(await manager.count(ComputedVote, {
          where: {
            conference_id,
            n_vote: max_n_vote,
          },
     ...
     *  return tasks
     */
    async update_lifecycle(block_end_timestamp: number) {
      ...
      for (const proposal of await manager.find(ProposalEntity, {
        ...
        for (const conference of conferences) {
          ...
          for (const computed_vote of computed_votes) {
            const solution = await manager.findOne(
              SolutionEntity,
        ...
        if (
          ...
          await set_proposal_status(proposal.id, ProposalStatus.publicizing);
          const conference_id = (
      ...
      for (const proposal of await manager.find(ProposalEntity, {
        ...
        const tasks: VITask[] = [];
        await set_proposal_status(proposal.id, ProposalStatus.executing);
        const tmp = await Promise.all(
          (
            await manager.find(SolutionTaskPair, {
              solution_id: proposal.computed_final_solution_id,
            })
          ).map((i) => manager.findOne(Task, i.task_id)),
        );
        for (const i of tmp) {
          tasks.push({
            type: i.type,
    ...
    async finish(proposal_id: ProposalID) {
      await set_proposal_status(proposal_id, ProposalStatus.done);
    },

    async get_solution(id: SolutionID) {
      const solution = await manager.findOne(SolutionEntity, id);
