...
import { Action, ActionType, Profile, ProposalID, ProposalStatus, ConsensusConfig, ConsensusEvent, BlockContext, BlockCtxState, VITaskType, WitnessTestimony, Actions, ActionBundle, ActionSubjects, ActionSignatures, InitialParams } from '../shared/types';
import { encode_to_str, get_now, b64pad_to_uint8array, RSA_sign, sleep, uint8array_to_b64 } from '../shared/utils';
...
describe('blockchain', function () {
  async function init_p2p(config_a: ConsensusConfig, config_b: ConsensusConfig) {
    if (!get_ready()) {
    ...
    const [profile_a, profile_b]: Profile[] = [
      {
        name: '王小红',
        public_key: config_a.peer_id_json.pubKey,
        proof_cid: 'x',
      },
      {
        name: '李小绿',
        public_key: config_b.peer_id_json.pubKey,
        proof_cid: 'b',
      },
    ...
    const peer_a = await PeerId.createFromJSON(config_a.peer_id_json);
    const peer_b = await PeerId.createFromJSON(config_b.peer_id_json);
    const use_a = profile_a.public_key === config_a.peer_id_json.pubKey;
    const initial_action_subjects: ActionSubjects = [
      use_a ? config_a.peer_id_json.id : config_b.peer_id_json.id,
    ];
  ...
  async function wait_actions_finished(peer: BlockChain) {
    ...
    await new Promise<void>((resolve) => {
      peer.on(ConsensusEvent.after_apply_actions, (stage: BlockContext) => {
        if (stage.epoch >= cycle_id && stage.actions.length) {
          resolve();
        }
      });
    });
  ...
  async function wait_cycle_id_finished(peer: BlockChain, cycle_id: number) {
    await new Promise<void>((resolve) => {
      peer.on(ConsensusEvent.witness_broadcast_end, (stage: BlockContext) => {
        if (stage.epoch >= cycle_id) {
          resolve();
        }
      });
    });
